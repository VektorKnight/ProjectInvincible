...
public class GlowController : MonoBehaviour
{
	...
	private List<GlowingObject> _glowableObjects = new List<GlowingObject>();
	...
	/// </summary>
	public static void RegisterObject(GlowingObject glowObj)
	{
		if (_instance != null)
		{
	...
	private void RebuildCommandBuffer()
	{
		...
		_commandBuffer.GetTemporaryRT(_prePassRenderTexID, Screen.width, Screen.height, 0, FilterMode.Bilinear, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Default, 1);
		_commandBuffer.SetRenderTarget(_prePassRenderTexID);
		_commandBuffer.ClearRenderTarget(true, true, Color.clear);

		for (var i = 0; i < _glowableObjects.Count; i++) {
			if (_glowableObjects[i] == null) continue;
